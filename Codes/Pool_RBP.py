...
for d in pooling_depth:
	...
	#print(pool)
	#pool = list(set(pool))
	# for key, values in doc_RBP.iteritems():
	# 	for item in values:
	# 		if item[2] not in initial_pool[item[0]].keys():
	# 		# print(item)
	# 			initial_pool[item[0]][item[2]] = [(1-prob)*prob ** (int(item[3])-1)]
	# 		for key1, values1 in doc_RBP.iteritems():
	# 			for item1 in values1:
	# 				if item1[2] in initial_pool[item1[0]].keys():
	# 					initial_pool[item1[0]][item1[2]] = max((1-prob)*prob ** (int(item1[3])-1),initial_pool[item1[0]][item1[2]])
	# 				else:
	# 					initial_pool[item1[0]][item1[2]] = [(1-prob)*prob ** (int(item1[3])-1)]
	print("next")
	for key1, values1 in doc_RBP.iteritems():
		for item1 in values1:
			if item1[2] in initial_pool[item1[0]].keys():
